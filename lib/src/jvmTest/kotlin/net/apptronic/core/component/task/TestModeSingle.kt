...
class TestModeSingle : BaseTestComponent() {
    ...
    private val taskScheduler = taskScheduler<Int>(SchedulerMode.Single) {
        onStart().onNext {
    ...
    fun verifyParallel() {
        ...
        assertResults(preProcessedItems, 1, 2)
        assertResults(postProcessedItems, 1)

        deferredWorker.runDeferredActions()

        assertResults(preProcessedItems, 1, 2, 3)
        assertResults(postProcessedItems, 1, 2)
